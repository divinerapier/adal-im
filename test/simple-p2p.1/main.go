...
func main() {
	// go setup(1, 2)
	user1, err := strconv.Atoi(os.Args[2])
	if err != nil {
		panic(err)
	}
	user2, err := strconv.Atoi(os.Args[3])
	if err != nil {
		panic(err)
	}
	setup(uint64(user1), uint64(user2))
}
...
func keepalive(user uint64, conn net.Conn) {
	...
	data = NewPacket(user, 4, data).Encode()
	for {
